...
pub fn run_coverage() {
    ...
    println!("Page routes (excluded):      {:>3}", report.page_routes.len());
    ...
    if !report.missing.is_empty() {
        eprintln!("\nError: {} data API routes are missing utoipa annotations.", report.missing.len());
        std::process::exit(1);
    }
...
pub fn generate_openapi_rs() {
    ...
    let mut annotated: Vec<AnnotatedRoute> = all_routes
        .into_iter()
        .filter(|r| !PAGE_MODULES.contains(&r.module_path.as_str()) && has_annotation(r, backend_root))
    ...
    // Stable sort by group: get, post, put, delete, fairing.
    let group_order: [&str; 5] = ["get", "post", "put", "delete", "fairing"];
    annotated.sort_by_key(|r| {
        group_order.iter().position(|&g| g == r.group_prefix).unwrap_or(99)
    });

    // ── Build output ──────────────────────────────────────────────────
    let mut out = String::new();

    writeln!(out, "// Auto-generated by `cargo xtask openapi-gen`. Do not edit manually.").unwrap();
    writeln!(out, "use utoipa::OpenApi;").unwrap();
    writeln!(out).unwrap();

    for r in &annotated {
        let import_mod = format!("crate::router::{g}::{m}", g = r.group_prefix, m = r.module_path);
        writeln!(out, "use {import_mod}::__path_{handler};", import_mod = import_mod, handler = r.handler).unwrap();
    }
